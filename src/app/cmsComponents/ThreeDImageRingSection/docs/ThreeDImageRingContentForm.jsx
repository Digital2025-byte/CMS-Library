...
export default function ThreeDImageRingContentForm({
  content,
  onChange,
  defaults,
}) {
      ...
      <InspectorSection title="Items" onReset={() => reset(ITEM_KEYS)}>
        ...
        >
          {(item, { index, update }) => (
            <>
              <InspectorField
                id={`three-d-image-ring-${index}-caption`}
                label="Caption"
                value={item.caption || ""}
                onChange={(value) => update("caption", value)}
              />
              <InspectorField
                id={`three-d-image-ring-${index}-image`}
                label="Image URL"
                value={item.imageUrl || ""}
                onChange={(value) => update("imageUrl", value)}
              />
              <InspectorField
                id={`three-d-image-ring-${index}-alt`}
                label="Image alt"
                value={item.imageAlt || ""}
