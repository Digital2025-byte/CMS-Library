...
/**
 * Lightswind ThreeDImageRing — original 3D carousel (drag + inertia).
 */
export function ThreeDImageRing({
  ...
  const angle = useMemo(
    () => (images.length > 0 ? 360 / images.length : 0),
    [images.length]
  );
  ...
  useEffect(() => {
    const handleResize = () => {
      setCurrentScale(
        window.innerWidth <= mobileBreakpoint ? mobileScaleFactor : 1
      );
    };
  ...
  const handleDragEnd = useCallback(() => {
    ...
    animate(initial, initial + velocityBoost, {
      ...
      restDelta: 0.5,
      modifyTarget: (target) =>
        angle ? Math.round(target / angle) * angle : target,
  ...
  const handleDragStart = useCallback(
    ...
    [draggable, handleDrag, handleDragEnd, rotationY]
  );
                  ...
                  <motion.div
                    ...
                    onHoverStart={() => {
                      ...
                      Array.from(ringRef.current.children).forEach(
                        (imgEl, i) => {
                          if (i !== index) {
                            imgEl.style.opacity = String(hoverOpacity);
                          }
                        }
                      );
                    }}
